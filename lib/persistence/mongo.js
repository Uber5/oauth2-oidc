  ...
  return {
    ...
    collections: {
      client: {
        ...
        createClient: function(name, redirect_uris) { // TODO: not required here, but may be used by dependant packages
          const client = {
            name,
            redirect_uris,
            key: crypto.randomBytes(12).toString('hex'),
            secret: crypto.randomBytes(40).toString('hex'),
            // enforceAuthOnTokenRequest: true // TODO: important default. Rather change logic to *not* have to set `true` as default?
          }
          return genericCreate('clients')(client)
        }
      },
      auth: {
